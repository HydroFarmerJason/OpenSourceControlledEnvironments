...
# ASCII Art Banner - Updated for v2.1
print_banner() {
    echo -e "${GREEN}"
    cat << "EOF"
 ...
 | |_| |___) | |___| |___ |_____/ __/ | |_| |
  ...
  Open Source Controlled Environments v2.1
...
# Detect hardware platform with v2.1 EM sensor support
detect_platform() {
...
# Check system requirements - Updated for v2.1
check_requirements() {
    log "Checking system requirements for OSCE v2.1..."
    
    # Check Python version (3.8+ required for v2.1)
    if command -v python3 &> /dev/null; then
        ...
        if [ "$PYTHON_MAJOR" -ge 3 ] && [ "$PYTHON_MINOR" -ge 8 ]; then
            success "Python $PYTHON_VERSION found (3.8+ required for v2.1)"
        else
            error "Python 3.8+ required for v2.1, found $PYTHON_VERSION"
            exit 1
        fi
    else
    ...
    # Check RAM (4GB recommended for v2.1)
    TOTAL_MEM=$(awk '/MemTotal/ {print $2}' /proc/meminfo)
    TOTAL_MEM_GB=$((TOTAL_MEM / 1024 / 1024))
    if [ "$TOTAL_MEM_GB" -lt 2 ]; then
        warning "Only ${TOTAL_MEM_GB}GB RAM detected. 2GB minimum, 4GB recommended for v2.1"
    else
    ...
    # Platform-specific checks for v2.1
    if [[ $PLATFORM == pi* ]]; then
        # Check for I2C
        if [ -e /dev/i2c-1 ]; then
...
# Install system dependencies - Updated for v2.1
install_dependencies() {
    log "Installing system dependencies for OSCE v2.1..."
    
    # Common dependencies
    DEPS="python3-venv python3-dev build-essential libssl-dev libffi-dev"
    DEPS="$DEPS python3-numpy python3-scipy"  # v2.1 requirements
...
# Create OSCE v2.1 directory structure
create_directories() {
    log "Creating OSCE v2.1 directory structure..."
...
# Create v2.1 core modules
create_core_modules() {
    log "Creating OSCE v2.1 core modules..."
...
EOQPA
    ...
    success "Core v2.1 modules created"
}

# Download or create minimal core
download_core() {
    log "Creating OSCE v2.1 core..."
...
#!/usr/bin/env python3
"""
OSCE v2.1 Minimal Core - Just Enough to Start
...
try:
    ...
    print(f"Warning: Could not import v2.1 modules: {e}")
...
class MinimalOSCE:
    """Minimal OSCE v2.1 implementation to get started"""
    ...
    def _init_v2_components(self):
        """Initialize v2.1 components if available"""
        if PHALCore:
            try:
    ...
    def _setup_routes(self):
        ...
        @self.app.route('/')
        def index():
            return '<h1>OSCE v2.1</h1><p>Dashboard: <a href="/dashboard">/dashboard</a></p>'
    ...
    def run(self, host='0.0.0.0', port=8080):
        """Start the OSCE v2.1 system"""
        logger.info(f"Starting OSCE v2.1 on {host}:{port}")
        logger.info(f"Dashboard: http://localhost:{port}/dashboard")
        logger.info(f"API: http://localhost:{port}/api/status")
        self.app.run(host=host, port=port, debug=False)

# Minimal dashboard template with v2.1 branding
DASHBOARD_TEMPLATE = '''
<!DOCTYPE html>
<html>
<head>
    <title>OSCE v2.1 - Open Source Controlled Environments</title>
...
<body>
    <div class="container">
        <div class="header">
            <h1> OSCE v2.1 - Planetary Harmony Edition</h1>
        ...
        <div class="status">
            ...
            <strong>Advanced Dashboard:</strong> Coming soon with full v2.1 features!
        </div>
    </div>
</body>
</html>
...
EOCORE
    ...
    if [ -f "osce-dashboard-v21.html" ]; then
        ...
        success "v2.1 Dashboard copied"
    else
        # Create minimal dashboard placeholder
        echo "<h1>OSCE v2.1 Dashboard - Full version coming soon!</h1>" > "$STATIC_DIR/dashboard.html"
    fi
...
EOSTART
    ...
    success "OSCE v2.1 core created"
}

# Setup Python virtual environment with v2.1 dependencies
setup_venv() {
    log "Setting up Python environment for v2.1..."
    ...
    success "Python environment ready for v2.1"
}

# Create systemd service for v2.1
create_service() {
...
[Unit]
Description=OSCE v2.1 - Open Source Controlled Environments with Planetary Harmony
...
Restart=always
RestartSec=10
Environment="OSCE_VERSION=2.1.0"
...
EOSERVICE
        ...
        if [ "$EUID" -ne 0 ]; then
            ...
            success "OSCE v2.1 service installed"
        fi
    fi
}
...
main() {
    ...
    echo "Welcome to OSCE v2.1 - The WordPress of Controlled Environment Agriculture"
    ...
    echo ""
    echo -e "${GREEN}════════════════════════════════════════════════════════════════${NC}"
    echo -e "${GREEN} OSCE v2.1 Installation Complete! ${NC}"
    echo -e "${GREEN}════════════════════════════════════════════════════════════════${NC}"
    echo ""
    ...
    echo ""
    echo -e "${PURPLE}v2.1 Features:${NC}"
